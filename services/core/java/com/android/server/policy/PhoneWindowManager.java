...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    // used for both screenshot and region screenshot
    ...
    private boolean mRegionScreenshotChordEnabled;
    private boolean mRegionScreenshotChordVolumeUpKeyConsumed;
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        if (interactive && !mPowerKeyTriggered
            ...
            interceptRegionScreenshotChord();
        }
        ...
        if (!mTorchEnabled || isScreenOn()) {
            ...
            case LONG_PRESS_POWER_GLOBAL_ACTIONS:
                ...
                boolean globalActionsOnLockScreen = Settings.System.getIntForUser(
                        mContext.getContentResolver(), Settings.System.POWER_MENU_LOCKSCREEN, 1, UserHandle.USER_CURRENT) == 1;
    ...
    private void interceptRegionScreenshotChord() {
        if (mRegionScreenshotChordEnabled
            ...
            if (now <= mVolumeUpKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
                ...
                mRegionScreenshotChordVolumeUpKeyConsumed = true;
                cancelPendingPowerKeyAction();
                mScreenshotRunnable.setScreenshotType(TAKE_SCREENSHOT_SELECTED_REGION);
                mHandler.postDelayed(mScreenshotRunnable, getScreenshotChordLongPressDelay());
            }
    ...
    private void cancelPendingRegionScreenshotChordAction() {
        mHandler.removeCallbacks(mScreenshotRunnable);
    }
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        mRegionScreenshotChordEnabled = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_enableRegionScreenshotChord);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        // but we're not sure, then tell the dispatcher to wait a little while and
        // try again later before dispatching.
        if (mRegionScreenshotChordEnabled && (flags & KeyEvent.FLAG_FALLBACK) == 0) {
            ...
            if (keyCode == KeyEvent.KEYCODE_VOLUME_UP
                    && mRegionScreenshotChordVolumeUpKeyConsumed) {
                if (!down) {
                    mRegionScreenshotChordVolumeUpKeyConsumed = false;
                }
                return -1;
            }
        }
                ...
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                    if (down) {
                        if (interactive && !mVolumeDownKeyTriggered
                            ...
                            mScreenshotChordVolumeDownKeyConsumed = false;
                            cancelPendingPowerKeyAction();
                            cancelPendingRegionScreenshotChordAction();
                            checkSettings();
                            interceptScreenshotChord();
                        }
                    } else {

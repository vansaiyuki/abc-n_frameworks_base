...
public class BatteryMeterDrawable extends Drawable implements
    ...
    private int getBoltColor() {
        if (mBoltOverlay) {
            return mContext.getResources().getColor((mStyle == BATTERY_STYLE_CIRCLE || mStyle == BATTERY_STYLE_BIGCIRCLE) ? R.color.batterymeter_bolt_color : R.color.system_primary_color);
        }
    ...
    private void drawBattery(Canvas canvas) {
        ...
        if (d instanceof BitmapDrawable) {
            ...
            bd.getPaint().set(!mPluggedIn || (mPluggedIn && mShowPercent && !mForceChargeBatteryText) ? mClearPaint : mTextAndBoltPaint);
            if (mBoltOverlay) {
                mBoltDrawable.setTint(getBoltColor());
            }
        } else {
